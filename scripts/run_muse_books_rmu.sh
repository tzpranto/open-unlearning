#!/bin/bash
# MUSE Books: RMU baseline
# Restart-safe: skips if final model exists (RMU uses max_steps, no epoch checkpoints)
set -euo pipefail
...
PROGRESS="/tmp/books_rmu_progress.log"
...
if compgen -G "${TASK_DIR}/model-*.safetensors" > /dev/null 2>&1; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        log "[TRAIN] ${TASK}: fresh (bs=4 accum=8 eff_bs=32, max_steps=80)"
    fi

    "${PYTHON}" src/train.py --config-name=unlearn.yaml \
        experiment=unlearn/muse/rmu \
        model=${MODEL} \
