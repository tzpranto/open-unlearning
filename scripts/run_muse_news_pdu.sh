#!/bin/bash
# MUSE News: PDU baseline (Primal-Dual Unlearning)
# Source: arXiv:2506.05314, Table 5
# Paper params: alpha=50, eps=1.5(News)/0.1(Books), warmup=3, dual_lr=1
...
if compgen -G "${TASK_DIR}/model-*.safetensors" > /dev/null 2>&1; then
    ...
    if [ -n "$LAST_CKPT" ]; then
        ...
        log "[TRAIN] ${TASK}: fresh (bs=4 accum=8 eff_bs=32)"
    fi

    "${PYTHON}" src/train.py --config-name=unlearn.yaml \
        experiment=unlearn/muse/pdu \
        model=${MODEL} \
