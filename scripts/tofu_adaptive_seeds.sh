#!/bin/bash
# LoRA-BiAL-Adaptive multi-seed runs — 5 seeds × 3 splits (1B)
# Same protocol as baselines: 5 seeds, results to CSV with mean±std
# Usage: nohup bash scripts/tofu_adaptive_seeds.sh > saves/unlearn/adaptive_seeds.log 2>&1 &
set -uo pipefail
...
CSV="docs/results/tofu_adaptive_seeds.csv"
...
get_retain_logs() {
    local split=$1
    local retain="${split/forget/retain}"
    # forget01->retain99, forget05->retain95, forget10->retain90
    case "$split" in
