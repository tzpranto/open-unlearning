#!/bin/bash
# BLADE hyperparam sweep — MUSE News, seed=42, 8 GPUs parallel
# 5 sweeps × 8 values × 2 (K=3, K=0) = 80 runs total
# Usage: bash scripts/sweep_muse_news.sh
set -uo pipefail
...
LOGDIR="saves/unlearn/sweep_muse_news"
...
# Verify target model exists (HF cached or local)
python -c "from transformers import AutoConfig; AutoConfig.from_pretrained('muse-bench/MUSE-News_target')" 2>/dev/null
if [[ $? -ne 0 ]]; then
    echo "[FATAL] Target model muse-bench/MUSE-News_target not accessible"
    exit 1
fi
...
run_single() {
    ...
    # Check if eval exists (from prior run or inline)
    ...
    # Train (includes inline eval for MUSE)
    ...
    # Find eval output (trainer runs eval inline)
