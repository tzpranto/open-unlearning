#!/bin/bash
# BLADE hyperparam sweep (K=0) — TOFU 1B forget01, seed=42, 8 GPUs parallel
# 5 sweeps × 8 values = 40 runs (K=0) total, done in 5 batches of 8
# Usage: bash scripts/sweep_tofu_1b_01_k0.sh
set -uo pipefail
...
REPORT="results/sweep_tofu_1b_01_k0.md"
LOGDIR="saves/unlearn/sweep_tofu_1b_01_k0"
...
for i, line in enumerate(lines):
    for val in values:
        task_name = f"k0_sweep_{sweep_name}_{val}"
...
run_batch() {
    ...
    for val in "${values[@]}"; do
        local task_name="k0_sweep_${sweep_name}_${val}"
