#!/bin/bash
# BLADE hyperparam sweep — KnowUnDo copyright, seed=42, 8 GPUs parallel
# 5 sweeps × 8 values × 2 (K=3, K=0) = 80 runs total
# Usage: bash scripts/sweep_knowundo_copyright.sh
set -uo pipefail
...
DOMAIN="copyright"
REPORT_K3="results/sweep_knowundo_copyright_k3.md"
REPORT_K0="results/sweep_knowundo_copyright_k0.md"
LOGDIR="saves/unlearn/sweep_knowundo_copyright"
...
find_eval_json() {
    local dir=$1
    # Trainer saves eval to checkpoint-0/evals/ or evals/ depending on version
    local candidates=(
...
run_single() {
    ...
    # Skip if both evals already exist but not in CSV (re-record)
    ...
    # Train + inline eval (skip if model already saved)
    ...
    # Verify model was saved (supports sharded models)
    if ! model_exists "$model_dir"; then
        echo "[GPU $gpu] TRAIN FAILED $task_name (no model saved)"
        return 1
    fi

    # Find KnowUnDo eval output (trainer runs eval inline)
    local eval_json=$(find_eval_json "$model_dir")
    if [[ -z "$eval_json" ]]; then
        # Eval didn't run inline — run separately
    ...
    # Clean model weights (sharded or single)
...
echo ""
echo "################################################################"
echo " PHASE 2: K=0 (40 runs)"
echo " Started: $(date '+%Y-%m-%d %H:%M:%S')"
echo "################################################################"
PREFIX="copyright_k0"
# Override K to 0 by appending to extra_args — need to modify run_single
# Actually, set a global K override
K_OVERRIDE="trainer.method_args.K=0"
...
# Override run_single for K=0 phase
run_single() { run_single_k0 "$@"; }
run_all_sweeps
