#!/bin/bash
# LoRA-BiAL with 20% importance-sampled PerTA init
# 20% PerTA over-forgets (fk=0.260) — more headroom for bilevel retain recovery
set -e
cd /datadrive/forked/open-unlearning

TASK="lora_bial_1ep_imp20"
SPLIT="News"
RETAIN_LOGS="saves/eval/muse_Llama-2-7b-hf_${SPLIT}_retrain/MUSE_EVAL.json"

echo "============================================="
echo "LoRA-BiAL + 20% PerTA: ${TASK}"
echo "============================================="

# Train — same as v1 but with 20% importance-sampled Fisher
python src/train.py --config-name=unlearn.yaml \
    ...
    trainer.method_args.fisher_cache_path=saves/unlearn/_perta_fisher_cache_News_imp20pct.pt
...
print(f'LoRA-BiAL imp20: fk={fk:.4f}, rk={rk:.4f}, delta={delta:+.3f}')
for k in sorted(d.keys()):
    if isinstance(d[k], dict) and 'agg_value' in d[k]:
        print(f'  {k}: {d[k][\"agg_value\"]:.4f}')
print()
print('References:')
print('  20% PerTA init: fk=0.260, rk=0.309, delta=-0.004')
print('  v1 step-300:   fk=0.349, rk=0.447, delta=+0.085')
