...
PDU collapses at 4$\times$ scale (HM=0.005, retain destroyed) and after 2 sequential steps (HM 0.58$\to$0.02). \method degrades gracefully: scalability HM stays 0.542--0.552 through 3$\times$, sustainability HM holds 0.542--0.525 through 4 sequential steps. The bounded loss and LoRA bottleneck prevent the accumulated representational damage that destroys PDU under repeated application.
...
Across all settings (\figref{fig:dynamics_blade}), \method follows a three-phase pattern:
...
PDU (\figref{fig:dynamics_pdu}) shows qualitatively different behavior: its unbounded logit-margin loss starts at 700+ and produces recurring retain spikes throughout training. The symmetric dual update offers no lasting protection---$\lambda$ drops as fast as it rises, producing oscillatory cycles that never resolve. These oscillations map directly to worse final metrics.
